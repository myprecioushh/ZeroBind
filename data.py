...
class FewShotBatchSampler:
    def __init__(self, train_molecule_datasets, K_shot, K_query, batch_size):
        ...
        while len(proteins) >= self.batch_size:
            random.seed(42)
            group_weights = [len(molecule_ID[protein]) for protein in proteins]
...
class GCNMoleculeDataModule(LightningDataModule):
    def __init__(self, num_workers, batch_size, k_shot, k_query, val_shot, test, explanation):
        ...
        if self.explanation:
            ...
            self.train_batch = FewShotBatchSampler(self.train_molecule, batch_size=self.batch_size, K_shot=self.k_shot,
